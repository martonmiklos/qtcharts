...
LineMarkerItem::LineMarkerItem(QLineMarker *marker, QGraphicsItem *item)
    : ChartItem(marker->d_func(), item),
      m_marker(marker)
{
    QObject::connect(marker->d_func(), SIGNAL(updated()), this, SLOT(handleUpdated()));
    handleUpdated();
}
...
void LineMarkerItem::handleUpdated()
{
    ...
    for (QAbstractAxis *attachedAxis : m_marker->attachedAxes()) {
    ...
    if (axis->orientation() == Qt::Horizontal) {
        m_line = QLineF(axis->min() - m_marker->pen().width()/2.0,
                        m_marker->position(),
                        axis->max() + m_marker->pen().width()/2.0,
                        m_marker->position());
        m_boundingRect.setHeight(m_marker->pen().width());
        m_boundingRect.setWidth(m_line.length());
    } else if (axis->orientation() == Qt::Vertical) {
        m_line = QLineF(m_marker->position(),
                        axis->min() - m_marker->pen().width()/2.0,
                        m_marker->position(),
                        axis->max() + m_marker->pen().width()/2.0);
